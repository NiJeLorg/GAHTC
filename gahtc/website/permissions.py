...
class Perms(DefaultPermissionHandler):
    """ 
    Custom permission handler based on default described below. Relplaced user.is_staff with user.is_active
    ...
    def filter_categories(self, user, qs):
        ...
        return qs.filter(hidden=False) if not user.is_active else qs

    def may_view_category(self, user, category):
        """ return True if `user` may view this category, False if not """
        return user.is_active or not category.hidden
    ...
    def filter_forums(self, user, qs):
        ...
        return qs.filter(Q(hidden=False) & Q(category__hidden=False)) if not user.is_active else qs

    def may_view_forum(self, user, forum):
        """ return True if user may view this forum, False if not """
        return user.is_active or ( forum.hidden == False and forum.category.hidden == False )
    ...
    def may_view_topic(self, user, topic):
        ...
        if user.is_superuser:
            return True
        if not user.is_active and (topic.forum.hidden or topic.forum.category.hidden):
    ...
    def may_create_post(self, user, topic):
        ...
        if topic.forum.hidden and (not user.is_active):
            # if topic is hidden, only staff may post
            return False

        if topic.closed and (not user.is_active):
            # if topic is closed, only staff may post
            return False

        # only user which have 'pybb.add_post' permission may post
        return defaults.PYBB_ENABLE_ANONYMOUS_POST or user.is_active
